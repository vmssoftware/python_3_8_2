...
#define _VMS_MAKE_INHERITABLE_(x, flag)                                         \
    if ((x) != -1) {                                                            \
        if (0 > _Py_set_inheritable_async_safe((int)(x), flag, NULL)) {         \
            PyErr_Clear();                                                      \
            errno = 0;                                                          \
        }                                                                       \
    }
...
static PyObject* make_fds_non_inheritable(PyObject *py_fds_to_keep) {
    int start_fd = 3;
    int end_fd = _MAX_FD_TO_CLOSE_;
    int len = PyTuple_GET_SIZE(py_fds_to_keep);
    PyObject *py_fds_to_make_inherit = PyList_New(0);
    for (int i = 0; i < len; ++i) {
        int keep_fd = PyLong_AsUnsignedLong(PyTuple_GET_ITEM(py_fds_to_keep, i));
           ...
           PyObject *preexec_fn_args_tuple)
{
    ...
    int pid = -1;
    int exec_error = 0;
    unsigned short ch_pid_mbx = 0;
    PyObject *py_fds_to_make_inherit = NULL;

    /* Make inheritable might block the thread on OpenVMS */
    ...
    if (close_fds) {
        ...
        ch_pid_mbx = cre_pid_mbx(getpid(), 0);
        if (ch_pid_mbx) {
            send_fds_to_keep(ch_pid_mbx, py_fds_to_keep);
        }
#else
        py_fds_to_make_inherit = make_fds_non_inheritable(py_fds_to_keep);
#endif
    }
...
#ifdef _SEND_FDS_MBX_
    if (ch_pid_mbx) {
        // wait only if we sent something
        int timeout = 3 * 10000;
        while(--timeout && simple_check_mbx(ch_pid_mbx) > 0) {
            vms_sleep(100);
        }
        del_pid_mbx(ch_pid_mbx, 0);
    }
#else
